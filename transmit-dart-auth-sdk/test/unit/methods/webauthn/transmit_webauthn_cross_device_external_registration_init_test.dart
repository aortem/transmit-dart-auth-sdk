...
import 'package:transmit_dart_auth_sdk/src/methods/webauthn/transmit_webauthn_cross_device_external_registration_init.dart';
...
void main() {
  group('AortemTransmitWebAuthnCrossDeviceExternalRegistrationInit', () {
    ...
    test('mockInitRegistration returns mock data with correct keys', () async {
      ...
      expect(result.containsKey('cross_device_ticket_id'), isTrue);
      expect(result['mock_data'], isTrue);
    });

    test('mockInitRegistration throws for an empty external user id', () {
      expect(
    ...
    test('mockInitRegistration throws for an empty username', () {
      expect(
        () => service.mockInitRegistration(
          externalUserId: 'user123',
          username: '',
