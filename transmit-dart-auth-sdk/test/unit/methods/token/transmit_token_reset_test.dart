...
import 'package:transmit_dart_auth_sdk/src/methods/token/transmit_token_reset.dart';
// Adjust package import as needed

void main() {
  group('resetPassword (HTTP Request Approach) Tests', () {
    final transmitPasswordReset = AortemTransmitTokenReset(
    ...
    test('throws error for empty reset token or new password', () {
      expect(
        () => transmitPasswordReset.resetPassword('', newPassword),
        throwsA(isA<ArgumentError>()),
      );
      expect(
        () => transmitPasswordReset.resetPassword(resetToken, ''),
        throwsA(isA<ArgumentError>()),
      );
    });
  });
  ...
  group('resetPasswordStub (Stub/Mock Implementation) Tests', () {
    ...
    test('throws error for empty inputs (stub)', () {
      expect(
        () => transmitPasswordReset.resetPasswordStub('', newPassword),
        throwsA(isA<ArgumentError>()),
      );
      expect(
        () => transmitPasswordReset.resetPasswordStub(resetToken, ''),
        throwsA(isA<ArgumentError>()),
      );
    });
  });
